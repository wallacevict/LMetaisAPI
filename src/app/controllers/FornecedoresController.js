...
class FornecedoresController {
    async index(req, res) {
        ...
        await Fornecedores.paginate(filter, { sort: { nome_uf: 1 }, page, limit }).then((list) => {
            return res.json({
                error: false,
                list
            })
    ...
    async show(req, res) {
        Fornecedores.findOne({ _id: req.params.id }, '-__v').then((type) => {
            return res.json({
                error: false,
                type
            })
    ...
    async store(req, res) {
        ...
        const type = await Fornecedores.create(dados, (err) => {
    ...
    async update(req, res) {
        ...
        const typeExist = await Fornecedores.findOne({ _id });
        ...
        await Fornecedores.updateOne({ _id: dados._id }, dados, (err) => {
            if (err) return res.status(400).json({
                error: true,
                code: 110,
    ...
    async delete(req, res) {
        ...
        await Fornecedores.updateOne({ _id: req.params.id }, dados, (err) => {
            if (err) return res.status(400).json({
                error: true,
                code: 110,
